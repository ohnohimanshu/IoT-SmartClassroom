...
def video_live_stream(request, pk):
    ...
    return StreamingHttpResponse(
        _generate_video_stream(video.video_file.path, camera_id=0,
                               camera_location='Video Playback',
                               request_obj=request),
        content_type='multipart/x-mixed-replace; boundary=frame',
    )
...
def analyze_video(video_pk):
    """
    Offline analysis of an uploaded classroom video.

    Colour coding (same as live stream):
      GREEN  — focused
      ORANGE — distracted / looking_away / head_down
      RED    — phone / eating / fighting  (face-recog run here too)
    """
    import traceback
    ...
    try:
        ...
        from classroom_monitor.behavior_detection import (
            ClassroomBehaviorDetector, COLOR_MAP, LABEL_MAP,
            ALERT_POSES, DISTRACTED_POSES, STORE_POSES
        )
        from classroom_monitor.face_recognition_helper import StudentFaceRecognizer, DLIB_LOCK

        detector   = ClassroomBehaviorDetector(camera_url='', camera_id=0)
        recognizer = StudentFaceRecognizer()
        recognizer.load_from_db()
        print(f'[OK] Detector + Recognizer ready for video {video_pk}')

        cap = cv2.VideoCapture(video.video_file.path)
        if not cap.isOpened():
            raise RuntimeError(f'Cannot open: {video.video_file.path}')
        ...
        frame_interval  = max(1, int(fps * 2))   # analyse 1 frame per 2 s
        frame_count     = 0
        analyzed_frames = 0
        ...
        while True:
            ...
            focused_count = looking_away_count = head_down_count = 0
            using_phone_count = eating_count = fighting_count = not_visible_count = 0
            zone_data = []
            annotated = frame.copy()
            ...
            for det in detections:
                ...
                if det_type == 'focused':           focused_count      += 1
                elif det_type == 'looking_away':    looking_away_count += 1
                elif det_type == 'head_down':       head_down_count    += 1
                elif det_type == 'using_phone':     using_phone_count  += 1
                elif det_type == 'eating_food':     eating_count       += 1
                else:                               not_visible_count  += 1

                x1, y1, x2, y2 = det['bbox']
                color  = det.get('color', COLOR_MAP.get(det_type, (120,120,120)))
                label  = det.get('label', LABEL_MAP.get(det_type, det_type))

                # Face recognition for ALL storable incidents (RED + ORANGE)
                name, roll, sid = 'Unknown', '', None
                if det['is_alert'] or det['is_distracted']:
                    crop = frame[y1:y2, x1:x2]
                    if crop.size > 0:
                        sid, name, roll, _ = recognizer.match(crop)

                # Save IncidentReport for RED + ORANGE incidents
                if det['is_alert'] or det['is_distracted']:
                    try:
                        from entrance_cam.models import Student as _Stu
                        _student = _Stu.objects.get(pk=sid) if sid else None
                    except Exception:
                        _student = None

                    # Save snapshot to file
                    _ts = timezone.now().strftime('%Y%m%d_%H%M%S_%f')
                    _, _buf = cv2.imencode('.jpg', annotated,
                                          [cv2.IMWRITE_JPEG_QUALITY, 80])
                    _snap_file = ContentFile(
                        _buf.tobytes(),
                        name=f'incident_{_ts}.jpg'
                    )
                    _severity = ('medium' if det['is_alert']
                                 else 'low')
                    _tag = f'{name} ({roll})' if sid else 'Unknown person'
                    _inc = IncidentReport.objects.create(
                        student=_student,
                        incident_type=det_type,
                        severity=_severity,
                        confidence=det.get('confidence', 0.0),
                        snapshot=_snap_file,
                        description=f'{label} — {_tag} (video analysis)',
                        whatsapp_sent=False,
                    )
                    # Email alert only for RED incidents
                    if det['is_alert']:
                        _, _buf2 = cv2.imencode('.jpg', frame,
                                               [cv2.IMWRITE_JPEG_QUALITY, 80])
                        _send_incident_email(
                            incident=_inc,
                            student=_student,
                            ...
                            roll_no=roll,
                            det_type=det_type,
                            snapshot_bytes=_buf2.tobytes(),
                        )

                # Draw box: thickness=3 for alert, 2 otherwise
                thickness = 3 if det['is_alert'] else 2
                cv2.rectangle(annotated, (x1, y1), (x2, y2), color, thickness)
                if name != 'Unknown':
                    cv2.putText(annotated, f'{name} ({roll})',
                                (x1, max(y1 - 22, 14)),
            ...
            total_detected   = (focused_count + looking_away_count + head_down_count
                                + using_phone_count + eating_count)
            total_students_sum += total_detected
            engagement_score = (focused_count / total_detected * 100) if total_detected > 0 else 0.0
            total_engagement += engagement_score

            # Summary bar
            bar = (f'Focused:{focused_count}  Distracted:{looking_away_count + head_down_count}'
                   f'  Phone:{using_phone_count}  Eating:{eating_count}  Score:{engagement_score:.0f}%')
            bar_w = min(len(bar) * 9 + 14, annotated.shape[1])
            cv2.rectangle(annotated, (0, 0), (bar_w, 26), (20, 20, 20), -1)
            cv2.putText(annotated, bar, (6, 18),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 255, 255), 1)
            ...
            legend = [('Focused', (0,200,60)), ('Distracted', (0,165,255)), ('Alert', (0,0,220))]
            for li, (ltxt, lclr) in enumerate(legend):
                lx = w - 130; ly = 12 + li * 20
                cv2.rectangle(annotated, (lx, ly-10), (lx+14, ly+4), lclr, -1)
                cv2.putText(annotated, ltxt, (lx+18, ly+3),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.40, lclr, 1)
            ...
            print(f'[Video {video_pk}] {timestamp:.1f}s — '
                  f'focused:{focused_count} '
                  f'distracted:{looking_away_count + head_down_count} '
                  f'phone:{using_phone_count} eating:{eating_count} '
                  f'fight:{fighting_count} not_visible:{not_visible_count} '
        ...
        video.save()
        print(f'[OK] Video {video_pk} done — {analyzed_frames} frames, avg {video.average_engagement_score:.1f}%')
...
def api_incidents_report(request):
    ...
    try:
        ...
        alert_types = {'using_phone', 'eating_food'}
        inc_type    = data.get('incident_type', 'other')
        severity    = 'medium' if inc_type in alert_types else 'low'
        ...
        # ── 6. Email alert for RED incidents ─────────────────────────────────
        if inc_type in ('using_phone', 'eating_food') and snapshot_bytes:
            _send_incident_email(
                incident=incident,
                student=student,
